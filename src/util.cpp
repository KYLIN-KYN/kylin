...
const char * const BITCOIN_CONF_FILENAME = "KYLIN.conf";
const char * const BITCOIN_PID_FILENAME = "KYLINd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "KYLIN" is a composite category enabling all KYLIN-related debug output
            if(ptrCategory->count(string("KYLIN"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\KYLINCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\KYLINCore
    // Mac: ~/Library/Application Support/KYLINCore
    // Unix: ~/.KYLINcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KYLINCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/KYLINCore";
#else
    // Unix
    return pathRet / ".KYLINcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty KYLIN.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override KYLIN.conf
